        ...
        <form method="GET" action="<?php echo URLROOT; ?>/AdminPaymentHistoryController/index" class="row g-3 align-items-end">
            ...
            <div class="col-6 col-sm-4 col-md-2">
                ...
                <select name="status" class="form-select bg-dark bg-opacity-50 text-white border-secondary border-opacity-25">
                    <option value="all" <?php echo $data['filters']['status'] == 'all' ? 'selected' : ''; ?>>Semua</option>
                    <option value="paid" <?php echo $data['filters']['status'] == 'paid' ? 'selected' : ''; ?>>Paid / Lunas</option>
                    <option value="unpaid" <?php echo $data['filters']['status'] == 'unpaid' ? 'selected' : ''; ?>>Unpaid</option>
            ...
            <div class="col-6 col-sm-4 col-md-2">
                <label class="form-label text-secondary small fw-semibold">Metode</label>
                <select name="payment_method" class="form-select bg-dark bg-opacity-50 text-white border-secondary border-opacity-25">
                    <option value="all" <?php echo $data['filters']['payment_method'] == 'all' ? 'selected' : ''; ?>>Semua</option>
                    ...
                    <option value="manual" <?php echo $data['filters']['payment_method'] == 'manual' ? 'selected' : ''; ?>>Manual (Tunai)</option>
                </select>
            </div>
            ...
            <div class="col-12 col-sm-4 col-md-2 d-flex gap-2">
                ...
                <a href="<?php echo URLROOT; ?>/AdminPaymentHistoryController/index" class="btn btn-outline-secondary border-opacity-25 flex-shrink-0" title="Reset Filter"><i class="bi bi-arrow-counterclockwise"></i></a>
            </div>
        </form>
    </div>
</div>
...
<ul class="nav nav-tabs border-secondary border-opacity-25 flex-nowrap" role="tablist" style="min-width: max-content;">
    <li class="nav-item" role="presentation">
        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#payments" type="button"><i class="bi bi-credit-card me-1 me-sm-2"></i><span class="d-none d-sm-inline">Pembayaran Online</span><span class="d-inline d-sm-none">Online</span></button>
    </li>
    <li class="nav-item" role="presentation">
    ...
    <div class="tab-pane fade show active" id="payments">
        <div class="card glass-card border-0 shadow-sm">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($data['payments'] as $payment): ?>
                                    <tr>
                                        <td class="ps-4 text-secondary small"><?php echo date('d M Y H:i', strtotime($payment->created_at)); ?></td>
                                        <td class="font-monospace text-info small"><?php echo htmlspecialchars($payment->invoice_number); ?></td>
                                        ...
                                        <td class="text-secondary small"><?php echo htmlspecialchars($payment->gateway_name ?: 'Kasir (Tunai)'); ?></td>
            ...
            <div class="card-footer bg-transparent border-secondary border-opacity-25 d-flex justify-content-between align-items-center py-3 flex-wrap gap-2 pagination-footer" style="display: none;">
                <div class="text-secondary small pagination-info">Menampilkan 0 - 0 dari 0 data</div>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($data['invoices'] as $invoice): ?>
                                    <tr>
                                        <td class="ps-4 font-monospace text-info small"><?php echo htmlspecialchars($invoice->invoice_number); ?></td>
                                        ...
                                        <td class="text-secondary small"><?php echo date('M Y', strtotime($invoice->billing_month . '-01')); ?></td>
                                        <td class="text-secondary small"><?php echo date('d M Y', strtotime($invoice->due_date)); ?></td>
                                        ...
                                        <td class="pe-4 text-end">
                                            <a href="<?php echo URLROOT; ?>/AdminInvoiceController/thermal/<?php echo $invoice->id; ?>" target="_blank" class="btn btn-sm btn-outline-info border-opacity-25" title="Cetak Thermal 80mm">
                                                <i class="bi bi-printer me-1"></i> <span class="d-none d-md-inline">Struk</span>
                                            </a>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                    ...
                    <table class="table table-dark table-hover mb-0 align-middle">
                        <thead class="text-secondary small text-uppercase border-secondary border-opacity-25">
                            <tr>
                                ...
                                <th class="pe-4 py-3 border-0">Keterangan</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($data['customerLogs'] as $log): ?>
                                    <tr>
                                        <td class="ps-4 text-secondary small"><?php echo date('d M Y H:i', strtotime($log->created_at)); ?></td>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($data['whatsappLogs'] as $wlog): ?>
                                    <tr>
                                        <td class="ps-4 text-secondary small"><?php echo date('d M Y H:i', strtotime($wlog->created_at)); ?></td>
                                        <td class="text-white fw-semibold"><?php echo htmlspecialchars($wlog->customer_name); ?></td>
                                        <td class="text-info small font-monospace"><?php echo htmlspecialchars($wlog->phone_number); ?></td>
                                        <td>
                                            <span class="badge bg-secondary bg-opacity-25 text-white">
                                                <?php echo htmlspecialchars(str_replace('_', ' ', $wlog->message_type)); ?>
                                            </span>
                                        </td>
                                        <td class="text-secondary small text-truncate" style="max-width: 200px;" title="<?php echo htmlspecialchars($wlog->message); ?>">
